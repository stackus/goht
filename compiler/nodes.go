...
func (n *RootNode) Source(tw *templateWriter) error {
	if _, err := tw.Write("// Code generated by hamlet - DO NOT EDIT.\n\n"); err != nil {
		return err
	}
...
func (n *ElementNode) Source(tw *templateWriter) error {
	if n.nukeOuterWhitespace {
		if _, err := tw.WriteStringLiteral(hamlet.NukeBefore); err != nil {
			return err
		}
	}
	...
	if n.nukeInnerWhitespace {
		if _, err := tw.WriteStringLiteral(hamlet.NukeAfter); err != nil {
			return err
		}
	}
...
func (n *ElementNode) renderAttributes(tw *templateWriter) error {
	if n.objectRef != nil {
		vName := tw.GetVarName()
		if _, err := tw.WriteIndent(`if ` + vName + ` := hamlet.ObjectID(`); err != nil {
			return err
		}
	...
	for _, key := range n.attributes.keys {
		...
		if attr.isDynamic {
			if _, err := tw.WriteIndent(`if _, __err = __buf.WriteString(hamlet.EscapeString(`); err != nil {
				return err
			}
	...
	if n.attributesCmd != "" {
		...
		if _, err := tw.WriteIndent(vName + `, __err = hamlet.BuildAttributeList(` + n.attributesCmd + ")\n"); err != nil {
			return err
		}
...
func (n *ElementNode) renderClass(tw *templateWriter) error {
	...
	if _, err := tw.WriteIndent(vName + ", __err = hamlet.BuildClassList("); err != nil {
		return err
	}
	for i, class := range n.classes {
		switch class.typ {
		case tObjectRef:
			if _, err := tw.Write("hamlet.ObjectClass(" + class.lit + ")"); err != nil {
				return err
			}
...
func writeFormattedText(tw *templateWriter, t token) error {
	if matches := reFmtText.FindStringSubmatch(t.lit); matches != nil {
		if _, err := tw.Write(`hamlet.FormatString("`); err != nil {
			return err
		}
...
func (n *TextNode) Source(tw *templateWriter) error {
	if n.isDynamic {
		...
		if _, err := tw.WriteIndent(`if ` + vName + `, __err = hamlet.CaptureErrors(`); err != nil {
			return err
		}
		if !tw.isUnescaped {
			if _, err := tw.Write(`hamlet.EscapeString(`); err != nil {
				return err
			}
		}
...
func (n *RenderCommandNode) Source(tw *templateWriter) error {
	...
	if _, err := tw.WriteIndent(".Render(hamlet.PushChildren(ctx, " + vName + "), __buf); __err != nil { return }\n"); err != nil {
		return err
	}
